...
<section id="search-content content">	
	<?php if ( have_posts() ) : ?>
        <h1 class="page-title"><?php _e( 'Search Results for: ', 'mammoth' ); ?><span><?php the_search_query(); ?></span></h1>
    ...
    <?php else : ?>
        <div class="post no-results not-found">
            <h2 class="post-title"><?php _e( 'Nothing Found', 'mammoth' ) ?></h2>
            <div class="post-content">
                <p><?php _e( 'Sorry, but nothing was found matching your search criteria. Please try again with a different search.', 'mammoth' ); ?></p>
                <?php get_search_form(); ?>						
            </div>
        </div>
    <?php endif; ?>			
